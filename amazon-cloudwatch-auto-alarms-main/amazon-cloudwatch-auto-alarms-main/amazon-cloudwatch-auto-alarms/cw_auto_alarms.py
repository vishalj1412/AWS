...
alarm_cpu_high_default_threshold = getenv("ALARM_CPU_HIGH_      ", "75")
...
alarm_disk_space_percent_free_threshold = getenv("ALARM_DISK_PERCENT_LOW_THRESHOLD", "25")
...
default_alarms = {
    'AWS/EC2': [
        {
            'Key': alarm_separator.join(
                [alarm_identifier, 'AWS/EC2', 'CPUUtilization', 'GreaterThanOrEqualToThreshold', '5m', 'Average']),
    ...
    'AWS/Lambda': [
        {
            'Key': alarm_separator.join(
                [alarm_identifier, 'AWS/Lambda', 'Errors', 'GreaterThanOrEqualToThreshold', '5m', 'Average']),
            'Value': alarm_lambda_error_threshold
        },
        {
            'Key': alarm_separator.join(
                [alarm_identifier, 'AWS/Lambda', 'Throttles', 'GreaterThanOrEqualToThreshold', '5m', 'Average']),
            ...
            },
            {
                'Key': alarm_separator.join(
                    [alarm_identifier, cw_namespace, 'Memory % Committed Bytes In Use', 'objectname', 'Memory',
                     'instance', 'GreaterThanOrEqualToThreshold', '5m', 'Average']),
        ...
        'Amazon Linux': [
            {
                'Key': alarm_separator.join(
                    [alarm_identifier, cw_namespace, 'disk_used_percent', 'device', 'xvda1', 'fstype', 'xfs', 'path',
                     '/', 'GreaterThanOrEqualToThreshold', '5m', 'Average']),
                'Value': alarm_disk_used_percent_threshold
            },
            {
                'Key': alarm_separator.join(
                    [alarm_identifier, cw_namespace, 'mem_used_percent', 'GreaterThanOrEqualToThreshold', '5m', 'Average']),
                'Value': alarm_memory_high_default_threshold
            }
        ],
        'Red Hat': [
